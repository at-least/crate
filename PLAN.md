...
> 版本 1.8 · 2026-08-29（D13；GDrive/Dropbox 核心 + 視窗化；Phase 4 ReplayGain、EQ/正增益、Widget 契約 + Android）
...
4. **GDriveProvider**（最後；需要 docs/gdrive-setup.md 的 3 個 Client ID——依 D11，申請延後到實際要上 production 前才做）：插進現有管線，UI 加帳號連結頁；同步管線接上 `RetryPolicy`（transient 退避 / auth 重授）。
   ...
   > 2026-08-29（同日）：**掃描視窗化上線（三平台對等）**——model.md §1.8 定案：scanner 輸入改為 `ByteSource { size, read(offset,len) }`，經 `ChunkedReader`（64 KiB 對齊 chunk、每 chunk 抓一次快取）存取；parser 只讀結構需要的位元組（ID3 跳過 APIC、FLAC 跳過 PICTURE、MP4 跳過 mdat、WAV 跳過 data；Ogg 的 OpusHead/`\x01vorbis` 釘前 64KB、最後 OggS 釘尾 64KB——頁最大 65307B 必在其中）。三實作同 chunk 算法 + 同 parser 存取序列 ⇒ 觸碰 chunk 集合一致，gdrive_cases 的 `provider.requests` 即 Range 請求數契約（原 7 案例小檔 1 chunk 請求數不變；新案例 `gdrive_windowed_scan` 以合成大檔——moov 在尾的 m4a／150KB 封面 FLAC／150KB APIC MP3——驗證各只抓 2 個 chunk）。26 個 scanner cases 與 6 個 sync cases 輸出 byte-identical 不變。順手收斂三實作的邊角差異（ilst/vorbis 同鍵第一個非空值勝、vorbis 無 key 忽略、box size < header 停止巡訪、wav fmt 越界不讀）。provider 介面 `readBytes` → `open(path) -> ByteSource?`；GDrive read = `Range: bytes=a-b`（206；200 整檔則本地裁切）；雲端讀取中 404 → 同靜默丟棄。
