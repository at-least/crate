...
class MuWidgetTest {
    ...
    fun `playing shows advanced position and duration`() {
        ...
        assertEquals(
            "▶ 1:33 / 3:33",
            MuWidget.progressLabel(base, now, base.displayState(now)),
        )
    }
    ...
    fun `paused keeps stored position`() {
        ...
        assertEquals("⏸ 1:23 / 3:33", MuWidget.progressLabel(paused, now, paused.displayState(now)))
    }
    ...
    fun `no duration omits total`() {
        ...
        assertEquals("▶ 1:23", MuWidget.progressLabel(noDur, now, noDur.displayState(now)))
    }

    @Test
    fun `position clamps to duration`() {
        val now = 1_700_000_600_000 // +600s，超過時長
        assertEquals("▶ 3:33 / 3:33", MuWidget.progressLabel(base, now, base.displayState(now)))
    }
}
