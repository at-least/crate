...
/// CrateMac 播放：AVQueuePlayer 佇列（無 remote command/audio session——選單列 app 直出聲）。
final class MacPlayer: NSObject, ObservableObject {
    ...
    /// 總增益（ReplayGain + preamp）與 EQ 一併交給 DSP；播放器音量固定 1。
...
/// CrateMac 音樂庫狀態（≈ CrateiOS AppModel 的 mac 版；引擎管線共用 CrateKit.SyncRunner）。
...
final class MacModel: ObservableObject {
    ...
    init() {
        ...
        do {
            db = try CrateDatabase(url: support.appendingPathComponent("mumac.db"))
        } catch {
