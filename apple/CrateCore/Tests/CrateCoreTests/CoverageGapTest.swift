...
final class CoverageGapTest: XCTestCase {
    ...
    func testLocalFolderProviderGuards() throws {
        XCTAssertEqual(
            [:],
            LocalFolderProvider(root: URL(fileURLWithPath: "/nonexistent-crate-coverage-xyz")).snapshot())
        XCTAssertNil(LocalFolderProvider(root: URL(fileURLWithPath: "/nonexistent-crate-coverage-xyz"))
            .readBytes("x.flac"))
    ...
    private func makeTempDir() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("crate-core-cov-\(UUID().uuidString)")
