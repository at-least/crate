...
/// iOS/macOS 兩個 app 共用（CrateiOS AppModel / CrateMac AppModel 皆以此驅動引擎）。
public final class SyncRunner {
    ...
    private let queue = DispatchQueue(label: "crate.sync", qos: .userInitiated)
    private var engine: SyncEngine?
    private var root: URL?

    public init(db: CrateDatabase, pinManager: PinManager) {
        self.db = db
