        ...
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 20) {
                if showsPicker {
                    ...
                    .labelsHidden()
                    .padding(.horizontal, CrateTheme.pageInset)
                    .padding(.top, 4)
                }
                if showsPlaylists {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        ForEach(Array(playlists.enumerated()), id: \.element.name) { i, pl in
                            ...
                            if i < playlists.count - 1 {
                                Divider().padding(.leading, CrateTheme.pageInset + 68)
                            }
                        }
                    }
                }
                if showsAlbums {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        LazyVGrid(
                            columns: [GridItem(.adaptive(minimum: 150), spacing: CrateTheme.gridSpacing)],
        ...
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 8) {
                ArtworkImage(key: album.id, url: model.artworkURL(for: album.id),
                             loader: model.artwork, cornerRadius: CrateTheme.radiusM)
        ...
        Button(action: onTap) {
            HStack(spacing: 12) {
                PlaceholderArt(key: "pl:" + playlist.name, symbol: "music.note.list")
                    ...
                    .clipShape(RoundedRectangle(cornerRadius: CrateTheme.radiusS, style: .continuous))
            ...
            .padding(.horizontal, CrateTheme.pageInset)
            .padding(.vertical, 8)
            .frame(minHeight: CrateTheme.hitTarget)
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
    }
    ...
    var body: some View {
        if let album = model.ui.albums.first(where: { $0.id == albumId }) {
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ArtworkImage(key: album.id, url: model.artworkURL(for: album.id),
                                 loader: model.artwork, cornerRadius: CrateTheme.radiusL)
                ...
                VStack(spacing: 0) {
                    HStack(alignment: .center, spacing: 16) {
                        PlaceholderArt(key: "pl:" + pl.name, symbol: "music.note.list")
                            ...
                            .clipShape(RoundedRectangle(cornerRadius: CrateTheme.radiusM, style: .continuous))
    ...
    var body: some View {
        ...
        HStack(spacing: 12) {
            ArtworkImage(key: key, url: player.nowTrack.map { model.artworkURL(for: $0.albumId) } ?? nil,
                         loader: model.artwork, cornerRadius: CrateTheme.radiusS)
